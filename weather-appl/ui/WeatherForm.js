...
export default class WeatherForm {
    ...
    constructor(parentId, cities, maxDays) {
        ...
        this.#cities = cities;
        this.#formData = {};//объявляем как пустой объект
        this.#buildForm();
        this.#setElements();
        this.#setHandlers();
    ...
    //функция которая должна получать управление на событии change - справа будет функциональный объект, его вызывает браузер на событии change
    #setHandlers() {
        this.#cityElement.onchange = this.#cityHandler.bind(this);
        // () => this.#cityHandler(); - тоже самое но хуже, функция чтобы привязать this (объект точка и функция - тогда функция получит this (объект класса))
        //вместо addEventListener - onchange, здесь прямой доступ без setter&getter
    ...
    #setSelectOptions() {
        ...
        //range - получаем массив, соответствующий диапазону
...
function setOptionItems(element, options, placeHolder) {
    ...
    //placeHolder - что мы сейчас выбираем (задаем вопрос пользователю)
...
// const arr =[];
// arr.length = 5;
// this.#formData.map(data => arr.push(data));
// return arr;
